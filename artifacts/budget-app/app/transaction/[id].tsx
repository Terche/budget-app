...
export default function TransactionDetailScreen() {
  ...
  const [groupId, setGroupId] = useState(
    transaction?.groupId ?? expenseGroups[0]?.id ?? "",
  );
  ...
  if (!transaction) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        <Text style={[styles.title, { color: colors.foreground, textAlign: "center", marginTop: 100 }]}>
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>Expense Group</Text>
      <View style={styles.chipList}>
        {expenseGroups.map((g) => (
          <TouchableOpacity
            ...
            style={[
              styles.chip,
              { backgroundColor: groupId === g.id ? g.color + "22" : colors.muted, borderColor: groupId === g.id ? g.color : colors.border, borderWidth: 1 },
            ]}
            onPress={() => {
              setGroupId(g.id);
          ...
          >
            <Text style={{ color: groupId === g.id ? g.color : colors.mutedForeground, fontSize: 13, fontFamily: "Inter_500Medium" }}>
              {g.name}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <View style={styles.chipList}>
        {categories.filter((c) => !groupId || c.groupId === groupId).map((c) => (
          <TouchableOpacity
            key={c.id}
            style={[
              styles.chip,
              { backgroundColor: categoryId === c.id ? c.color + "22" : colors.muted, borderColor: categoryId === c.id ? c.color : colors.border, borderWidth: 1 },
            ]}
            onPress={() => setCategoryId(c.id)}
          >
            <Text style={{ color: categoryId === c.id ? c.color : colors.mutedForeground, fontSize: 13, fontFamily: "Inter_500Medium" }}>
              {c.name}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <FormField label="Notes" value={notes} onChangeText={setNotes} placeholder="Optional notes..." multiline numberOfLines={3} />
